...
namespace IL3DN
{
    ...
    {
        [FormerlySerializedAs("m_IsWalking")] [SerializeField]
        private bool mIsWalking;

        [FormerlySerializedAs("m_WalkSpeed")] [SerializeField]
        private float mWalkSpeed = 2;

        [FormerlySerializedAs("m_RunSpeed")] [SerializeField]
        private float mRunSpeed = 5;

        [FormerlySerializedAs("m_RunstepLenghten")] [SerializeField] [Range(0f, 1f)]
        private float mRunstepLenghten = 0.7f;

        [FormerlySerializedAs("m_JumpSpeed")] [SerializeField]
        private float mJumpSpeed = 5;

        [FormerlySerializedAs("m_StickToGroundForce")] [SerializeField]
        private float mStickToGroundForce = 10;

        [FormerlySerializedAs("m_GravityMultiplier")] [SerializeField]
        private float mGravityMultiplier = 2;

        [FormerlySerializedAs("m_MouseLook")] [SerializeField]
        private IL3DnSimpleMouseLook mMouseLook;

        [FormerlySerializedAs("m_StepInterval")] [SerializeField]
        private float mStepInterval = 2;

        [FormerlySerializedAs("m_FootstepSounds")] [SerializeField]
        private AudioClip[] mFootstepSounds; // an array of footstep sounds that will be randomly selected from.

        [FormerlySerializedAs("m_JumpSound")] [SerializeField]
        private AudioClip mJumpSound; // the sound played when character leaves the ground.

        [FormerlySerializedAs("m_LandSound")] [SerializeField]
        private AudioClip mLandSound; // the sound played when character touches back on ground.
        ...
        private void Update()
        {
            ...
            if (!_mCharacterController.isGrounded && !_mJumping && _mPreviouslyGrounded) _mMoveDir.y = 0f;
        ...
        private void ProgressStepCycle(float speed)
        {
            if (_mCharacterController.velocity.sqrMagnitude > 0 && (_mInput.x != 0 || _mInput.y != 0))
                _mStepCycle +=
                    (_mCharacterController.velocity.magnitude + speed * (mIsWalking ? 1f : mRunstepLenghten)) *
                    Time.fixedDeltaTime;

            if (!(_mStepCycle > _mNextStep)) return;
        ...
        {
            if (!_mCharacterController.isGrounded) return;
            if (!_isInSpecialSurface)
            {
            ...
            else
            {
                ...
                if (n >= _footStepsOverride.Length) n = 0;
